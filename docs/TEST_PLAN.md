...
| Refresh reconciliation | `tests/completed-refresh-outcome-store.test.cjs`、`tests/memory-runtime-authority-boundaries.test.cjs` | 完成 outcome 在返回前不可变发布；新进程只读找回精确 checkpoint/generation；完整 tuple、digest、Ed25519 验签；unknown/mismatch/tamper/ambiguity/symlink fail closed；first-use 与 existing store 路径/字节/mode 零变化；不读私钥、不 scan、不打开 SQLite、不写日志或 receipt |
...
`npm test` 运行 source/policy/unit contract；真实 Electron 发布门由 `npm run test:electron-release` 单独执行，并依次运行 governance 与 bounded renderer DOM behavior E2E。两类 Electron 探针共享跨平台 executable resolver，以 probe 模式禁用 GPU、隐藏窗口并隔离 `userData` / `CODEX_HOME`；visual behavior e2e 会通过真实 preload/main IPC seed fixture，reload renderer，再点击真实导航并检查项目卡片/项目详情、Tools 卡片安全边界、Memory/Experience curation UI、retrieval explanation DOM、老线程平台支持状态和 archive candidate / host-bridge queue 边界文案。该 e2e 会跑默认桌面窗口和 980px 窄桌面窗口，并对 Projects、Tools、Memory、Smart Optimization、archive candidate 面板做横向溢出检查。它仍不是完整人工视觉验收，也不能证明 Thread History Vault、所有 Agent retrieval 排序、Codex 宿主侧栏归档、clean-machine 安装路径或端到端真实用户工作流已经完成行为级验收。
...
- `npm run memory:gate` 是 CI、`npm test`、Mac 打包和 release-candidate verification 的强制默认门，执行冻结的 v2 release corpus 与 executor。corpus 同时保存独立的 documents 和 relevance judgments；runner 只把 documents 交给 executor，单 case executor 输入不含 expected IDs/tags/continuity，随后由 gate 独立计分。corpus ID 与 canonical SHA-256 固定，替换语料必须 fail closed。
- `npm run memory:evaluate:production-fixture` 运行 v1 deterministic production-path fixture，只能输出 `TEST_FIXTURE` 且 `productReleaseEligible=false`；`npm run memory:evaluate:synthetic` 只输出 `EVALUATION_ONLY`。两者都不能出现产品 release PASS。所有本地 benchmark 均不得读取用户数据库、raw session、凭据或网络。
- 本地正式门为 `node scripts/run-memory-release-benchmark.cjs --real --corpus benchmarks/memory-production-release-corpus.v2.json --executor benchmarks/memory-production-release-executor.cjs`；receipt 必须包含零 error/empty/partial、真实 latency/token、execution cases SHA-256，以及执行前后稳定的 HEAD/status/corpus/executor/harness/Runtime/lockfile/toolchain 绑定。缺少执行器、绑定漂移、预填 verdict/metrics、未批准 corpus 或执行异常必须 `NOT_RUN`/失败且非零退出。
...
- Run `npm run test:artifact-evidence` in CI to fault-test the deterministic local full-artifact manifest. The test must reject complete bundle and `app.asar` extras/missing/hash drift, lockfile/input drift, and any attempt to promote an unsigned local candidate to `signed-distribution`.
- Run `npm run test:candidate-evidence` to prove a content-addressed dirty candidate binds HEAD, exact NUL porcelain SHA, all dirty path postimages, package-lock/toolchain, test/benchmark/artifact manifests and commands. Focused negatives must change tracked bytes, untracked bytes, index-only status, manifest paths and stored claims; every change must invalidate verification. The candidate receipt must remain ignored and must never imply commit, tag, signing, notarization, installation or public release.
- For an already-built release candidate, run `npm run verify:release-candidate`. This separately verifies selected source, generates the external full-artifact manifest, and re-verifies the exact candidate without packaging, installation, network, signing, or notarization. A green result is `full-artifact`, never public-release or signed-distribution evidence.
