...
Agent Runtime Monitor 是知匣的本地 agent 运行监控模块。Codex 使用 session metadata；OpenClaw 已支持按需、bounded、read-only 的 `sessions.json + task_runs` metadata；Claude Code、Cursor/Windsurf/Gemini CLI 仍是 process-only planned adapter。所有平台都隐藏敏感命令行 token，不读取 session body，也不把进程采样冒充为精确线程归因。
...
## 10. OpenClaw 辅助执行与记忆边界（2026-07-20）

- OpenClaw 是 CEO Flow 后面的受控执行端，不是第二个 CEO；不得改变 CEO 模型、推理强度、权限、验收或发布规则。
- 知匣是唯一项目记忆权威；OpenClaw 只保留可丢弃的任务级 session 状态，不维护 `MEMORY.md`、原生 FTS/vector 记忆索引或长期 promotion。每个 task id 使用新 session，换 session 时由知匣重新提供 compact Hot/Warm packet，而不是复制旧聊天。
- 任务前必须经过 `retrieve_context(task_dispatch)`；任务后必须先由 Codex 验收 typed receipt，只有 accepted 且 source-backed 的 compact evidence 才能进入 `writeback_evidence`。
- 默认使用独立 `~/.openclaw-ceoflow` 状态、并发 1、无 heartbeat、无原生 memory search、无 session-memory hook、无 compaction memory flush、无 `memory-core`、无个人 channel/plugin/session 继承。CEO Flow bridge 每次执行前检查这些开关，配置漂移时 fail-closed。R0 本地只读路径已验证；R1 写代码必须另过 prepared-worktree、工具权限和中立复审门。
- 知匣只按需读取 OpenClaw `sessions.json` 与任务 SQLite 的固定元数据列，不读 JSONL 正文、task prompt、错误正文、图片/base64、凭据或 memory chunk，不递归扫描 4GB 主状态。

完整合同和性能预算见 `docs/OPENCLAW_MEMORY_CONTEXT_INTEGRATION.md`。

OpenClaw 旧记忆在完成退役后进入知匣 cold archive：显式构建一次脱敏 FTS 索引，Codex 审计线程可按关键词有界查询；普通项目召回不读取该索引。OpenClaw 审计任务由 CEO Flow 先从知匣取包，再注入摘要和 provider-safe sourceRefs，OpenClaw 不安装知匣 Skill、不直接读 Vault，也不重新启用自身记忆。
...
## 12. P0 Semantic Memory Graph (2026-08-01)

- The semantic knowledge graph is an additive Memory Runtime provider in app-owned `memory-runtime-index.sqlite`. The sql.js `memory_graph_nodes` / `memory_graph_edges` tables remain compatibility and activation metadata and are not migrated or replaced.
- `retrieve_context` automatically attempts exact-project semantic recall from `taskGoal`, returns at most 12 evidence-backed one-hop paths and about 1200 graph tokens, and treats an empty graph result as a normal fallback to existing Hot/Warm retrieval.
- Semantic entities bind scope, normalized exact project path, stable project identity, kind, canonical name, aliases, status, source refs, provenance/confidence, timestamps, and content hash. Relations bind exact endpoints, predicate, source refs, provenance, confidence, status/validity, optional `factId`, timestamps, and content hash.
- `memory_facts` remains the only typed fact/claim ledger. Semantic relations may point to a fact ID; no second free-text fact store is introduced.
- P0 extraction is deterministic and explicit-only: bounded project/item/source identity, tags, supported frontmatter relation fields, wikilinks, Markdown links, accepted source-backed writeback, and structured MemoryFacts. It makes no LLM call and performs no raw-body, session, Vault, or all-document scan.
- On `retrieve_context`, the provider may populate an empty graph from at most 24 items already present in the compact authority-filtered packet. Only exact-project accepted/current items with safe source refs are eligible; candidate/review/stale/global/foreign items and body-bearing, raw-session, secret, base64, or oversized structures fail closed. This is request-time metadata adaptation, not a workspace scan or rebuild.
- The verified app-owned CLI `operation=retrieve`, including the repository Skill invoker's packaged-source route, uses the same adapter, sidecar, graph query, and authority non-effects. Its semantic supplement is included inside the requested whole-packet token budget and 32 KB byte limit; graph paths are trimmed first. A physical installed `app.asar` remains a post-package commander gate. Advisory headless fallback remains non-authoritative.
