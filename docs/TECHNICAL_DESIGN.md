...
## App-Owned Headless Recovery

`electron/memoryRuntimeCli.cjs` exposes strict-JSON `scan`, `seed`, `verify`, `retrieve`, and explicit `write_compatibility` operations without opening Electron. Recovery authority uses the full app-owned Memory Core, its private signing state, authority receipts, and continuity policy. A scan receipt binds stable project identity, exact baseline HEAD, and bounded canonical source hashes; verification rescans before every current/readiness claim. Linked worktrees at the same HEAD and source hashes inherit canonical project memory, while changed HEAD/source hashes and foreign projects fail stale. `docs/Example Project_CURRENT_CHECKPOINT.md` is an explicit priority source and cannot be displaced by alphabetical scan limits.

Packaged helper and advisory headless tables remain `fallback_stale`, `current=false`, and `recoveryReady=false`; only the app-owned CLI can produce a verified readiness claim. `codex-skills/zhixia-local-docs/scripts/invoke-app-memory-runtime.cjs` finds either the source CLI or the packaged `app.asar` CLI and invokes it in Node mode without opening the UI.

Thread recovery is continuity-first. It always composes current summary, phase, accepted progress, open task, next action, thread lineage, original product goal, and an architecture anchor before relevance-based fill. The complete strict-JSON packet uses an adaptive 2200-token preferred budget with a user-approved 10000-token hard ceiling and a 48 KB byte ceiling. When the minimum five Hot/Warm anchors do not fit, the CLI grows through bounded steps and reports the attempted/effective budget; `strictTokenBudget=true` disables growth. It reserves a small graph allowance and trims graph paths first if the combined packet is over budget. Repeated empty sourceRef fields are omitted, while canonical title/path/hash and signed authority receipt identity remain. Raw sessions, generated giant packets, image/base64 payloads, credentials, and long logs are excluded.

Explicit reseed treats `original_goal` as a singleton long-term anchor. A changed source-backed goal supersedes the former accepted goal; it is not silently co-accepted and does not bypass continuity conflict detection. `write_compatibility` requires exact project identity SHA and current scan SHA, copies existing generated packets into app-owned hash-manifest backups, then writes bounded `project-resume.md`, `retrieval-packet.md`, and `thread-recovery-packet.json`. Generated files remain non-authoritative.

## Semantic Memory Graph Sidecar

The P0 semantic graph is implemented by `electron/semanticMemoryGraphPolicy.cjs` and `electron/memoryRuntimeIndexStore.cjs`. It uses two additive native SQLite tables in `memory-runtime-index.sqlite`:

- `semantic_memory_entities`: deterministic ID, `scope`, exact normalized `projectPath`, derived or explicit `projectId`, `kind`, `canonicalName`, aliases, status, source refs, provenance, confidence, timestamps, `contentHash`, and bounded lexical `searchTerms`.
- `semantic_memory_relations`: deterministic ID, exact scope/project identity, from/to entity IDs, predicate, source refs, provenance, confidence, status, validity range, optional `factId`, timestamps, and `contentHash`.

Schema creation and indexes run inside an immediate transaction. Existing compatible schemas are read without migration; missing tables/indexes are added idempotently. Missing columns, wrong affinity/NOT NULL contracts, or a non-TEXT sole primary key fail with `SEMANTIC_GRAPH_INCOMPATIBLE_SCHEMA`; the runtime never drops, renames, copies, or rebuilds an incompatible table.

Runtime project entities use an epoch `createdAt` independent of retrieval subsets. Every entity/relation replacement first reloads the existing row's `createdAt`, renormalizes the incoming record with that immutable value, and only then compares or writes `contentHash`. Therefore a retained-createdAt/hash mismatch from an earlier build self-heals on the next targeted seed/upsert; no table scan or destructive repair migration is used.

Accepted MemoryFact writeback graphifies scalar typed facts after the fact transaction succeeds. The relation references the existing fact ID. `supports` and `contradicts` can remain active only while that fact is active/accepted, exact-project, source-backed, and safe. Claims, inferred relations, missing-source active records, and all global records are forced to review. Superseded fact occurrences create superseded graph relations rather than reopening old current paths.

`retrieveMemoryRuntimeContext` invokes `retrieveSemanticGraphPaths` for every task-time context request. Immediately before lookup it passes at most 24 already-retrieved packet items through `buildSemanticGraphSeedFromRuntimeItems` and idempotently upserts eligible records. Eligibility requires an exact project path (or the verified Memory Core project identity on the CLI route), accepted/current status, no human-review requirement, and normalized source refs. The adapter reads only title/name, bounded summary aliases, existing tags/frontmatter tags, kind/status, timestamps, and source refs; safe normalized ref titles plus basename/stem may be aliases. Body/contentText/transcript/log fields and unsafe structures are rejected. It performs zero document enumeration, workspace/Vault/generated-knowledge scans, or additional canonical scans.

Electron derives the existing shared `ProjectIdentityEnvelope` for the requested project and uses its `canonicalRoot` for semantic seed/query identity. Only the envelope's canonical root and active `worktreeRoot` are accepted as candidate scopes; linked absolute refs are converted to stable relative metadata without file reads. Missing/mismatched envelopes fall back to the exact requested path with bounded diagnostics and cannot import another envelope's graph.

The verified `memoryRuntimeCli.cjs operation=retrieve` path uses the same adapter, sidecar upsert, and one-hop query after its existing canonical verification and authoritative Memory Core candidate composition. The repository `invoke-app-memory-runtime.cjs` regression exercises its source/packaged-source routing; a physical installed `app.asar` is intentionally deferred to the post-package commander gate. Both routes return compact seed written/unchanged/rejected diagnostics and a `semantic_graph_recall` receipt without storing task text. When Memory Core is unready, CLI retrieve still returns the bounded zero-hit graph shape and a non-persisted in-packet receipt, but performs no semantic seed/write and leaves `current=false/recoveryReady=false`. Query caps remain 96 candidates, 12 paths, and 1200 graph tokens in Electron; CLI caps are smaller and additionally constrained by remaining whole-packet budget. Graph results carry explicit authority non-effects and never modify `current`, `recoveryReady`, authority verification, continuity, or acceptance. Advisory headless fallback remains `fallback_stale` and need not claim graph currentness.

### On-Demand Project Graph View

The renderer uses Cytoscape.js as a lazily mounted visualization layer. `MemoryGraphExplorer` owns one Cytoscape instance, runs a bounded non-animated layout, and destroys the instance on graph replacement or unmount. Search and kind filters operate on the already-returned local nodes; they do not issue per-keystroke IPC queries. The graph chunk is loaded only when the project `graph` tab is rendered.

The UI route calls only the read-only `memoryRuntime:getSemanticGraphView` IPC. It does not prewarm `retrieve_context`; graph population remains part of normal task-time Memory Runtime retrieval. The handler reads `semantic_memory_entities` and `semantic_memory_relations` from `memory-runtime-index.sqlite`, applies exact `ProjectIdentityEnvelope` ownership and node/edge caps, maps only compact metadata/sourceRefs, and never calls the legacy sql.js graph synchronization or `saveDatabase/db.export()`. `ProjectIdentityEnvelope.projectId` is the primary storage scope; older path-hash rows can be read only when the canonical path is identical, with an explicit compatibility diagnostic and no migration write.

The default response is capped at 72 nodes and 160 edges. It contains query duration, partial/cap diagnostics, and explicit `readOnly/metadataOnly/no raw session/no Vault scan` performance claims. Empty and partial results are normal product states. Graph selection cannot mutate facts, promote candidates, set continuity slots, or change Memory Core authority.
