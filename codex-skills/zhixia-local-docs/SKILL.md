...
When the `zhixia-control` Codex Plugin is installed, prefer its app-owned MCP tools for visible local control: `open_app`, `scan_workspace`, `verify_project`, `retrieve_context`, `prepare_takeover`, `writeback_evidence`, and `refresh_binding`. `scan_workspace`, `verify_project`, and lifecycle writes open or focus the installed Mac app by default; compact retrieval and takeover stay headless unless `showApp=true`. Opening the UI never relaxes authority rules. Do not use desktop clicking to impersonate a successful Runtime receipt.
